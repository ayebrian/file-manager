...
Для авторизации под Администратором, используйте: ```Логин: admin``` ```Пароль: admin```. При использовании __StormGuardian Files__ на вашем сервере или хостинге, рекомендуется изменить логин и пароль - ```/php/class/CheckSession.php``` > ```SESSION_USERS```.
